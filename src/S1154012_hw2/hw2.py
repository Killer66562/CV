...
def opening(image: cv2.typing.MatLike, rad: int = 1) -> cv2.typing.MatLike:
    return dilate(erode(image, rad), rad)

def closing(image: cv2.typing.MatLike, rad: int = 1) -> cv2.typing.MatLike:
    return erode(dilate(image, rad), rad)
...
def main():
    ...
    lena_opening = opening(lena_binary)
    cv2.imwrite('lena_opening.png', lena_opening)

    lena_closing = closing(lena_binary)
